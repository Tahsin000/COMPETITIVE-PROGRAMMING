...
void solve()
{
    ll n; cin >> n;
    for(ll i=0; i<n; i++){
        ll p1x1, p1x2, p1y1, p1y2;
        cin >> p1x1 >> p1y1 >> p1x2 >> p1y2;
        ll p2x1, p2x2, p2y1, p2y2; 
        cin >> p2x1 >> p2y1 >> p2x2 >> p2y2;
        // cout << bool(p2x1 < p1x1)  << endl;
        if (i > 0) cout << "\n";    
        if ((max(p1x1 ,p2x1) < min(p1x2 , p2x2)) && (max(p1y1 ,p2y1) < min(p1y2 , p2y2))){
            cout << max(p1x1 ,p2x1) << " " << max(p1y1 ,p2y1) << " " << min(p1x2 , p2x2) << " " << min(p1y2 , p2y2) << endl;
        }
        else {
            cout << "No Overlap\n";
        }
