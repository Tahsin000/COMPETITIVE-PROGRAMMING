...
void solve()
{
    // Mon Nov 15 21:38:38 2021
    // END TIME  : "Tue Nov 16 00:10:02 2021"


    while(1){
        ll node, edge; cin >> node ;
        if (node == 0) break;
        cin >> edge;
        std::vector<ll> adj[40000];
        std::vector<ll> level(40000, -1);
        for(ll i=0; i<edge; i++){
            ll u, v; cin >> u >> v;
            adj[u].pb(v);
            adj[v].pb(u);
