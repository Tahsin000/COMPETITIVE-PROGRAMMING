...
std::vector<ll> sumV, sumSV;
/* Start Your FUN*/
void fun(ll n, std::vector<ll> v)
{
    sumV.assign(n , 0);
    sumV[0] = v[0];
    for(ll i=1; i<n; i++){
...
void solve()
{
    ll n, k;
    cin >> n >> k;
    std::vector<ll> v(n); for(ll i=0; i<n; i++) cin >> v[i];fun(n, v);
    std::vector<pair<ll , ll>> cnt;
    for(ll i=0; i<n-k+1; i++){
        ll sum = 0;
        ll l = i ;
        ll r = i + k;
        sum = (sumV[i+k-1] - sumV[i]) + v[i];
        // cout << sum << endl;
        cnt.pb({sum, i});
    }
    // for(auto x : cnt) cout << x.ff << " ";
    sort(all(cnt));
    cout << cnt[0].ss+1 << endl;
}
/* Start Your MAIN*/
int main()
{
